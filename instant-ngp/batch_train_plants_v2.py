...
def copy_plant_data(plant_name):
    ...
    if source_images.exists():
        ...
        log(f"  ✓ 复制 images_rgba -> images ({num_images} 张图像)")
    else:
...
def convert_colmap_to_text(plant_name):
    """将 COLMAP 二进制文件转换为文本格式"""
    log(f"转换 COLMAP 二进制文件为文本格式...")

    plant_dir = Path(TARGET_DIR) / plant_name
    sparse_bin = plant_dir / "sparse" / "0"
    sparse_txt = plant_dir / "colmap_text"

    # 检查二进制文件是否存在
    if not (sparse_bin / "cameras.bin").exists():
        log(f"  ✗ 未找到 COLMAP 二进制文件")
        return False

    # 创建文本输出目录
    sparse_txt.mkdir(exist_ok=True)

    # 运行 colmap model_converter
    cmd = [
        "colmap", "model_converter",
        "--input_path", str(sparse_bin),
    ...
    try:
        ...
        log(f"  ✗ 未找到 colmap 命令，请安装 colmap")
        return False
...
def generate_transforms_json(plant_name):
    ...
    text_dir = plant_dir / "colmap_text"
        ...
        if result.returncode == 0 or output_file.exists():
            ...
            if output_file.exists():
                with open(output_file, 'r') as f:
                    ...
                    aabb_scale = data.get('aabb_scale', 'N/A')
                    log(f"  ✓ 包含 {num_frames} 帧, aabb_scale={aabb_scale}")
                return True
            else:
...
def main():
    ...
    for i, plant in enumerate(PLANTS, 1):
        ...
        # 2. 转换 COLMAP 二进制到文本
        if not convert_colmap_to_text(plant):
            stats["failed"].append((plant, "COLMAP 转换失败"))
            continue
        stats["converted"] += 1

        # 3. 生成 transforms.json
    ...
    log(f"COLMAP 转换成功: {stats['converted']}")
