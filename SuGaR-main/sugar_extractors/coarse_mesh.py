...
def extract_mesh_from_coarse_sugar(args):
    ...
    if args.mesh_output_dir is None:
        if len(args.scene_path.replace('\\', '/').split("/")[-1]) > 0:
            args.mesh_output_dir = os.path.join("./output/coarse_mesh", os.path.basename(args.scene_path))
        else:
            args.mesh_output_dir = os.path.join("./output/coarse_mesh", os.path.basename(os.path.dirname(args.scene_path)))
    ...
    if not use_marching_cubes:
            ...
            for surface_level in surface_levels:
                ...
                for decimation_target in decimation_targets:
                    ...
                    if use_vanilla_3dgs:
                        ...
                        sugar_mesh_path = 'sugarmesh_' + sugar_checkpoint_path.replace('\\', '/').split('/')[-2].replace('sugarcoarse_', '') + '_levelZZ_decimAA.ply'
            ...
            with torch.no_grad():
                ...
                for decimation_target in decimation_targets:
                    ...
                    if use_vanilla_3dgs:
                        ...
                        sugar_mesh_path = 'sugarmesh_' + sugar_checkpoint_path.replace('\\', '/').split('/')[-2].replace('sugarcoarse_', '') + '_poissoncenters_decimAA.ply'
                    sugar_mesh_path = sugar_mesh_path.replace(
                            'AA', str(decimation_target).replace('.', '')
                            )
                    sugar_mesh_path = os.path.join(mesh_output_dir, sugar_mesh_path)
        ...
        if use_vanilla_3dgs:
            ...
            sugar_mesh_path = 'sugarmesh_' + sugar_checkpoint_path.replace('\\', '/').split('/')[-2].replace('sugarcoarse_', '') + 'marchingcubes_levelZZ_decimAA.ply'
        sugar_mesh_path = sugar_mesh_path.replace(
            'ZZ', str(surface_level).replace('.', '')
            ).replace(
                'AA', str(decimation_target).replace('.', '')
