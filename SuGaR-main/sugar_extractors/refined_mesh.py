...
def extract_mesh_and_texture_from_refined_sugar(args):
    ...
    _refined_parts = refined_model_path.replace('\\', '/').split('/')
    if args.n_gaussians_per_surface_triangle is None:
        n_gaussians_per_surface_triangle = int(_refined_parts[-2].split('_gaussperface')[-1])
    else:
        n_gaussians_per_surface_triangle = args.n_gaussians_per_surface_triangle
    
    # --- Output parameters ---
    if args.mesh_output_dir is None:
        if len(args.scene_path.replace('\\', '/').split("/")[-1]) > 0:
            args.mesh_output_dir = os.path.join("./output/refined_mesh", os.path.basename(args.scene_path))
        else:
            args.mesh_output_dir = os.path.join("./output/refined_mesh", os.path.basename(os.path.dirname(args.scene_path)))
    mesh_output_dir = args.mesh_output_dir
    os.makedirs(mesh_output_dir, exist_ok=True)
    
    mesh_save_path = _refined_parts[-2]
    ...
    _source_parts = source_path.replace('\\', '/').split('/')
    scene_name = _source_parts[-2] if len(_source_parts[-1]) == 0 else _source_parts[-1]
    sugar_mesh_path = os.path.join('./output/coarse_mesh/', scene_name, 
                                _refined_parts[-2].split('_normalconsistency')[0].replace('sugarfine', 'sugarmesh') + '.ply')
