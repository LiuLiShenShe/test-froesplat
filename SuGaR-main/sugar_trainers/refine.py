...
def refined_training(args):
    ...
    if args.output_dir is None:
        if len(args.scene_path.replace('\\', '/').split("/")[-1]) > 0:
            args.output_dir = os.path.join("./output/refined", os.path.basename(args.scene_path))
        else:
            args.output_dir = os.path.join("./output/refined", os.path.basename(os.path.dirname(args.scene_path)))
    ...
    if export_ply_at_the_end:
        ...
        tmp_list = model_path.replace('\\', '/').split('/')
