...
def coarse_training_with_sdf_regularization(args):
    ...
    if args.output_dir is None:
        if len(args.scene_path.replace('\\', '/').split("/")[-1]) > 0:
            args.output_dir = os.path.join("./output/coarse", os.path.basename(args.scene_path))
        else:
            args.output_dir = os.path.join("./output/coarse", os.path.basename(os.path.dirname(args.scene_path)))
