...
# -*- coding: utf-8 -*-
"""
Phase 15 — Challenge GT processing (adapted from 处理GT_v2.py).
...
and produces:
  ...
  GT_QA/gt_audit_challenge.csv
  GT_QA/challenge_QA_visual/<sample>/<frame>_qa.png

Same target definition as the Phase 12 easy set: potted_clean = potted & ~cube.

Usage:
    python process_gt_challenge.py [--gt_src /data/fj/F2DMAS/03-GT-区分_challenge]
"""
...
DELIVER = Path(__file__).resolve().parent.parent  # 阶段十五根目录
CLEAN_DIR = DELIVER / "GT_potted_clean_challenge"
...
def main() -> None:
    ...
    ap.add_argument("--gt_src", type=str,
                    default="/data/fj/F2DMAS/03-GT-区分_challenge")
    args = ap.parse_args()
    ...
    print(f"  输出→ GT_potted_clean_challenge/, GT_QA/")
