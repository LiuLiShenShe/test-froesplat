...
# Check distorted and undistorted camera params for CaoMei1
for subdir in ['distorted/sparse', 'sparse/0']:
    path = f'D:\\CAAS\\04-COLMAP\\CaoMei1\\{subdir}\\cameras.bin'
    if os.path.exists(path):
        cams = read_cameras_bin(path)
        for cid, cam in cams.items():
            print(f"{subdir}: cam{cid} model={cam['model']} {cam['w']}x{cam['h']} params={cam['params']}")
    else:
