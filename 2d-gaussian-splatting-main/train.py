...
def maybe_prune_gaussians(gaussians, opt, iteration, model_path, cameras=None):
    ...
    if max_remove > 0 and requested > max_remove:
        candidate_score = prune_score.clone()
        ...
        limited[topk] = True
        prune_mask = torch.logical_and(prune_mask, limited)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            pruning_report = maybe_prune_gaussians(gaussians, opt, iteration, scene.model_path, scene.getTrainCameras())
            if pruning_report is not None and pruning_report.get("removed", 0) > 0:
                print(
                    "\n[ITER {}] Plant-aware pruning removed {}/{} Gaussians ({:.2%})".format(
                        iteration,
                ...
                if iteration in saving_iterations:
                    print("\n[ITER {}] Re-saving Gaussians after Plant-aware pruning".format(iteration))
                    scene.save(iteration)
